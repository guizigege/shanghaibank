...
const projectType = process.env.APP_TYPE === 'client' ? 'pages/**' : 'admin/pages/**'

const appEntry = {
  app: resolve('./src/main.ts')
}
const pageSrc = [`/${projectType}/main.ts`, '/common/pages/**/main.ts'];
const pagesEntry = getEntry(resolve('./src'), pageSrc)
...
baseWebpackConfig = merge(baseWebpackConfig, {
  plugins: [
    new CopyWebpackPlugin([{
      from: path.resolve(`./config/app.${process.env.APP_TYPE}.json`),
      to: path.resolve(config.build.assetsRoot, 'app.json'),
      force: true
    }])
  ]
})

console.log('app type:', process.env.APP_TYPE);
